...
std::vector<std::vector<RelativeIndex>> SearchServer::search(const std::vector<std::string> &requests) {
    ...
    for (size_t requestIndex = 0; requestIndex < requests.size(); requestIndex++) { // REQUESTS
        result.push_back(searchOneRequest(requests[requestIndex]));
    }
    return result;
}
...
std::vector<RelativeIndex> SearchServer::searchOneRequest(const std::string &requests) {
    ...
    for (size_t symbolIndex = 0; symbolIndex < requests.length(); symbolIndex++) { // THE SAME PARSING ALGORITHM AS IN InverterIndex

        if (((requests[symbolIndex] >= '0' && requests[symbolIndex] <= '9') ||
             (requests[symbolIndex] >= 'A' && requests[symbolIndex] <= 'Z') ||
             (requests[symbolIndex] >= 'a' && requests[symbolIndex] <= 'z') ||
             (requests[symbolIndex] == '\'')) && (symbolIndex != requests.length() - 1)) {
            requestWord += (requests[symbolIndex]);
        } else {
            if (requestWord == "") continue;
            if ((symbolIndex == requests.length() - 1) && ((requests[symbolIndex] >= '0' && requests[symbolIndex] <= '9') ||
            (requests[symbolIndex] >= 'A' && requests[symbolIndex] <= 'Z') ||
            (requests[symbolIndex] >= 'a' && requests[symbolIndex] <= 'z'))) {
                requestWord += (requests[symbolIndex]);
            }
